...
var (
	eventQueue = make(chan Envelope, 100)

	// Epoch state management maps
...
func (c *ClientImpl) startListener() {
	go func() {
		for {
			...
			eventQueue <- Envelope{Type: msgType, From: from, Epoch: epoch, GroupID: groupID, Data: data}
		}
	}()
}
...
func (c *ClientImpl) StartEventLoop() {
	for env := range eventQueue {
		switch env.Type {
