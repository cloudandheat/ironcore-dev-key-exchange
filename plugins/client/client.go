...
func (c *ClientImpl) startListener() {
	go func() {
		for {
			...
			// Network completely down (Server container offline)
			if err != nil {
...
func (c *ClientImpl) processCommit(groupID string, env Envelope) {
	...
	if env.Epoch == expectedEpoch {
		...
		msgData, _ := json.Marshal(AppMessage{
			...
			GroupID: groupID,
			SPI:     rand.Uint32(),
			Address: "2001:0db8:85a3:0000:0000:8a2e:0370:7334", // TODO: replace example-address be Address coming from dpservice
		})
		// logrus.WithFields(nil).Info("################################# response for commit ", msgData)
		// logrus.WithFields(nil).Info("################################# have secret: ", secret)
...
func (c *ClientImpl) handleEvent(env Envelope) {
	...
	case "app_message":
		...
		fmt.Printf("[%s] (Decrypted via MLS) Message from %s: Action='%s', SPI=%d, Address=%s\n",
			c.name, env.From, appMsg.Action, appMsg.SPI, appMsg.Address)
	}
}
